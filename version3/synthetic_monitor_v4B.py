...
class StepObserver:
    ...
    def pause_timer(self):
            """
            Pause timing (can be nested safely)
            """
            if self.start_time is None:
                return
...
def build_url_metrics(registry, time_formatter):
    metrics = {}

    metrics["duration"] = Gauge(
        f"synthetic_url_duration_{time_formatter.label}",
        "Last URL duration",
        ["env", "scenario", "run_id"],
        registry=registry
    )
    ...
    metrics["fcp"] = Gauge(
        f"synthetic_url_fcp_{time_formatter.label}",
        "Last URL FCP",
        ["env", "scenario", "run_id"],
        registry=registry
    )
    ...
    metrics["lcp"] = Gauge(
        f"synthetic_url_lcp_{time_formatter.label}",
        "Last URL LCP",
        ["env", "scenario", "run_id"],
        registry=registry
    )
    ...
    metrics["run_ts"] = Gauge(
        "synthetic_run_last_push_timestamp",
        "Last push timestamp of synthetic run",
        ["env", "scenario", "run_id"],
        registry=registry
    )
...
def build_journey_metrics(registry, time_formatter):
    metrics = {}

    metrics["journey_duration"] = Gauge(
        f"synthetic_journey_duration_{time_formatter.label}",
        "Last journey duration",
        ["env", "journey", "run_id"],
        registry=registry
    )

    metrics["step_duration"] = Gauge(
        f"synthetic_step_duration_{time_formatter.label}",
        "Last step duration",
        ["env", "journey", "step", "run_id"],
        registry=registry
    )

    metrics["step_fcp"] = Gauge(
        f"synthetic_step_fcp_{time_formatter.label}",
        "Step First Contentful Paint",
        ["env", "journey", "step", "run_id"],
        registry=registry
    )

    metrics["step_lcp"] = Gauge(
        f"synthetic_step_lcp_{time_formatter.label}",
        "Step Largest Contentful Paint",
        ["env", "journey", "step", "run_id"],
        registry=registry
    )

    metrics["step_cls"] = Gauge(
        f"synthetic_step_cls",
        "Step Cumulative Layout Shift",
        ["env", "journey", "step", "run_id"],
        registry=registry
    )
    ...
    metrics["failure"] = Counter(
        ...
        "Journey failure count",
        ["env", "journey", "run_id"],
        registry=registry
    )

    metrics["run_ts"] = Gauge(
        f"synthetic_run_last_push_timestamp",
        "Last push timestamp of synthetic run",
        ["env", "journey", "run_id"],
        registry=registry
    )
...
def run_url_mode(args, base_dir, run_id, time_formatter):
    ...
    metrics = build_url_metrics(registry, time_formatter)
            ...
            while time.time() < end_time:
                for url in urls:
                    ...
                    if converted_duration >= 0:
                        metrics["duration"].labels(args.env, url, run_id).set(converted_duration)
                    if converted_fcp >= 0:
                        metrics["fcp"].labels(args.env, url, run_id).set(converted_fcp)
                    if converted_lcp >= 0:
                        metrics["lcp"].labels(args.env, url, run_id).set(converted_lcp)
                    if isinstance(cls, (int, float)) and cls >= 0:
                        metrics["cls"].labels(args.env, url, run_id).set(cls)
                    metrics["run_ts"].labels(args.env, url, run_id).set(time.time())
...
def run_journey_mode(args, base_dir, run_id, time_formatter):
    ...
    registry = CollectorRegistry()
    metrics = build_journey_metrics(registry, time_formatter)
                ...
                for journey_name, fn in JOURNEYS.items():
                    for row in data_rows:
                        ...
                        metrics["journey_duration"].labels(
                                ...
                                ).set(time_formatter.convert(total_duration))
                        ...
                        for step in steps:
                            if step["duration_ms"] >= 0:
                                metrics["step_duration"].labels(
                                    ...
                                    journey_name,
                                    step["step"],
                                    run_id).set(time_formatter.convert(step["duration_ms"]))
                            if step["fcp"] >= 0:
                                metrics["step_fcp"].labels(args.env, journey_name, step["step"], run_id).set(time_formatter.convert(step["fcp"]))
                            if step["lcp"] >= 0:
                                metrics["step_lcp"].labels(args.env, journey_name, step["step"], run_id).set(time_formatter.convert(step["lcp"]))
                        ...
                        metrics["run_ts"].labels(args.env, journey_name, run_id).set(time.time())
