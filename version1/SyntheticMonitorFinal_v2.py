...
def run_url_mode(args, base_dir, run_id, time_formatter):
    ...
    # ---- URL Duration Metrics ----
    url_avg = Gauge(
        f"synthetic_url_duration_avg_{time_formatter.label}",
        "Average URL load duration",
        ["env", "scenario", "run_id"],
        registry=registry
    )

    url_p90 = Gauge(
        f"synthetic_url_duration_p90_{time_formatter.label}",
        "P90 URL load duration",
        ["env", "scenario", "run_id"],
        registry=registry
    )

    url_max = Gauge(
        f"synthetic_url_duration_max_{time_formatter.label}",
        "Max URL load duration",
        ["env", "scenario", "run_id"],
        registry=registry
    )

    url_min = Gauge(
        f"synthetic_url_duration_min_{time_formatter.label}",
        "Min URL load duration",
        ["env", "scenario", "run_id"],
        registry=registry
    )

    url_samples = Gauge(
        "synthetic_url_samples_total",
        "Total successful URL samples",
        ["env", "scenario", "run_id"],
        registry=registry
    )
    ...
    url_success = Gauge(
        ...
        "Total successful URL executions",
        ["env", "scenario", "run_id"],
        registry=registry
    )
    ...
    for scenario, values in timings.items():
        ...
        url_avg.labels(args.env, scenario, run_id).set(
            time_formatter.convert(avg_val)
        )
        url_p90.labels(args.env, scenario, run_id).set(
            time_formatter.convert(p90_val)
        )
        url_max.labels(args.env, scenario, run_id).set(
            time_formatter.convert(max_val)
        )
        url_min.labels(args.env, scenario, run_id).set(
            time_formatter.convert(min_val)
        )
        url_samples.labels(args.env, scenario, run_id).set(len(values))
        url_success.labels(args.env, scenario, run_id).set(len(values))

    # Failures = total attempts - successes
    for url in urls:
        total_attempts = len(timings.get(url, []))
        url_failure.labels(args.env, url, run_id).set(0)  # If needed, track separately

    push_to_gateway(
        "localhost:9091",
        job="synthetic_url_monitor",
        registry=registry
    )
...
def run_journey_mode(args, base_dir, run_id, time_formatter):
    ...
    # ---- Journey Metrics ----
    journey_avg = Gauge(
        f"synthetic_journey_duration_avg_{time_formatter.label}",
    ...
    journey_p90 = Gauge(
        f"synthetic_journey_duration_p90_{time_formatter.label}",
        "P90 journey duration",
        ["env", "journey", "run_id"],
        registry=registry
    )

    journey_samples = Gauge(
        "synthetic_journey_samples_total",
        "Total journey samples",
        ["env", "journey", "run_id"],
        registry=registry
    )
    ...
    # ---- Step Metrics ----
    step_avg = Gauge(
        f"synthetic_step_duration_avg_{time_formatter.label}",
    ...
    step_p90 = Gauge(
        f"synthetic_step_duration_p90_{time_formatter.label}",
        "P90 step duration",
        ["env", "journey", "step", "run_id"],
        registry=registry
    )
    ...
    step_samples = Gauge(
        "synthetic_step_samples_total",
        "Total step samples",
        ["env", "journey", "step", "run_id"],
        registry=registry
    )

    # ---- Populate Journey Metrics ----
    for journey_name, values in journey_timings.items():
        if not values:
            continue
        ...
        ).set(time_formatter.convert(statistics.mean(values)))

        journey_p90.labels(
            args.env, journey_name, run_id
        ).set(time_formatter.convert(percentile(values, 90)))

        journey_samples.labels(
            args.env, journey_name, run_id
        ).set(len(values))
    ...
    for (journey_name, step_name), values in step_timings.items():
        ...
        step_avg.labels(
            args.env, journey_name, step_name, run_id
        ).set(time_formatter.convert(statistics.mean(valid_values)))

        step_p90.labels(
            args.env, journey_name, step_name, run_id
        ).set(time_formatter.convert(percentile(valid_values, 90)))

        step_samples.labels(
            args.env, journey_name, step_name, run_id
        ).set(len(valid_values))

    push_to_gateway(
        "localhost:9091",
        job="synthetic_journey_monitor",
        registry=registry
    )
