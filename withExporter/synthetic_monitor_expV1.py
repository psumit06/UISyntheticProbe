...
def parse_args():
    ...
    p.add_argument("--rel-name",required=True,help="Please enter Release/Run Name")
...
#  Production UX latency buckets
LATENCY_BUCKETS_S = tuple(x / 1000.0 for x in (
    100, 250, 500, 750,
    1000, 1500, 2000,
    3000, 4000, 5000,
...
def load_journey_data(path):
    """
    Safe loader:
    - CSV provided → load normally
    - CSV missing → return single empty row so journeys without data still run
    """
    if not path:
        return [{}]

    if not os.path.exists(path):
        print(f"[WARN] Journey data file not found: {path}. Running with empty data.")
        return [{}]
    ...
    try:
        ...
        print(f"[WARN] Failed to read journey data ({e}). Running with empty data.")
        return [{}]
...
def build_url_metrics():
    return {
        ...
        "success": Counter(
            "synthetic_url_success_total",
            "URL success",
            ["env", "scenario", "run_id"]
        ),
        "failure": Counter(
            "synthetic_url_failure_total",
            "URL failure",
            ["env", "scenario", "run_id"]
        ),
    }
...
def build_journey_metrics():
    return {
        "journey_duration": Histogram(
            "synthetic_journey_duration_s",
            "Journey duration in seconds",
            ["env", "journey", "run_id"],
            buckets=LATENCY_BUCKETS_S,
        ),
        "step_duration": Histogram(
            "synthetic_step_duration_s",
            "Step duration in seconds",
            ["env", "journey", "step", "run_id"],
            buckets=LATENCY_BUCKETS_S,
        ),
        "step_fcp": Histogram(
            "synthetic_step_fcp_s",
            "Step FCP in seconds",
            ["env", "journey", "step", "run_id"],
            buckets=LATENCY_BUCKETS_S,
        ),
        "step_lcp": Histogram(
            "synthetic_step_lcp_s",
            "Step LCP in seconds",
            ["env", "journey", "step", "run_id"],
            buckets=LATENCY_BUCKETS_S,
        ),
        ...
        "success": Counter(
            "synthetic_journey_success_total",
            "Journey success",
            ["env", "journey", "run_id"]
        ),
        "failure": Counter(
            "synthetic_journey_failure_total",
            "Journey failure",
            ["env", "journey", "run_id"]
        ),
    }
        ...
        with sync_playwright() as playwright:
            ...
            while time.time() < end_time:
                for url in urls:
                    ...
                    if duration > 0:
                        metrics["duration"].labels(args.env, url, run_id).observe(duration / 1000.0)
                    if fcp > 0:
                        metrics["fcp"].labels(args.env, url, run_id).observe(fcp / 1000.0)
                    if lcp > 0:
                        metrics["lcp"].labels(args.env, url, run_id).observe(lcp / 1000.0)
...
## Preload datasets
    ...
    for journey_name, meta in JOURNEYS.items():

        dataset_paths = meta.get("data")
        join_type = meta.get("data_join", "concat")

        if args.journey_data:
            dataset_paths = [args.journey_data]

        if not dataset_paths:
            journey_datasets[journey_name] = [{}]
            continue

        datasets = [load_journey_data(p) for p in dataset_paths]
        ...
        # -------- CROSS JOIN --------
            ...
            for combo in itertools.product(*datasets):
                merged = {}
                for r in combo:
                    merged.update(r)
                rows.append(merged)
        ...
        # -------- RANDOM JOIN --------
            ...
            rows = datasets   # store datasets, choose later
        ...
        journey_datasets[journey_name] = rows if rows else [{}]
        ...
        with sync_playwright() as playwright:
            ...
            while time.time() < end_time:
                ...
                for journey_name, meta in JOURNEYS.items():
                    ...
                    fn = meta["fn"]
                    dataset_rows = journey_datasets[journey_name]
                    join_type = meta.get("data_join", "concat")
                    ...
                    for row in rows_to_run:
                        ...
                        # ===============================
                        # PAGE + OBSERVER (correct lifecycle)
                        # ===============================
                        page = ctx.new_page()
                        ...
                        try:
                            ...
                            for step_data in steps:
                                ...
                                # ---------- Bucketing ----------
                                if step_data["duration_ms"] and step_data["duration_ms"] > 0:
                                ...
                                lcp_val = step_data.get("lcp", -1)
                                if isinstance(lcp_val, (int, float)) and lcp_val >= 0:
                                    bucketed_step_lcp[(journey_name, step_data["step"])][bucket_start].append(lcp_val)
                        ...
                        if total_duration > 0:
                            ...
                            metrics["journey_duration"].labels(args.env, journey_name, run_id).observe(total_duration / 1000.0)
                        ...
                        for step in steps:
                            if step["duration_ms"] > 0:
                                metrics["step_duration"].labels(
                                    ...
                                    journey_name,
                                    step["step"],
                                    run_id).observe(step["duration_ms"] / 1000.0)
                            if step["fcp"] > 0:
                                metrics["step_fcp"].labels(args.env, journey_name, step["step"], run_id).observe(step["fcp"] / 1000.0)
                            if step["lcp"] > 0:
                                metrics["step_lcp"].labels(args.env, journey_name, step["step"], run_id).observe(step["lcp"] / 1000.0)
                        ...
                        journey_writer.writerow([
                            ...
                            journey_name,
                            status,
                            time_formatter.convert(total_duration),
                            str(row)
                        ])
...
def main():
    ...
    print(f"[INFO] Prometheus metrics exposed on :{METRICS_PORT}/metrics")
